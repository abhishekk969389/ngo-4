...
let content = fs.readFileSync('scratch/base_index.ts', 'utf8');

// Also, the base_index.ts has lines that access nonexistent properties (TaxBenefits, EventDetail, JobDetail, NotFound, ProgramDetail). 
// I should remove those specific lines so tsc doesn't complain about them.
const linesToExclude = ['TaxBenefits', 'EventDetail', 'JobDetail', 'NotFound', 'ProgramDetail'];
let newContent = content.split('\n').filter(line => {
    return !linesToExclude.some(term => line.includes(term));
}).join('\n');

fs.writeFileSync('app/data/index.ts', newContent + aliases);
