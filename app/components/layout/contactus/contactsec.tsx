...
const infoIconMap: Record<string, React.ComponentType<{ className?: string }>> = {
    phone: Phone,
    mail: Mail,
    'map-pin': MapPin,
};
                    ...
                    <div className="lg:col-span-5">
                        <div className="rounded-3xl border border-[#e5eae2] bg-[#f5f8f4] p-6 shadow-sm sm:p-8 lg:p-10">
                            ...
                            <div className="space-y-6">
                                {info.items.map((item: NgoContactInfoDetailItem, idx: number) => {
                                    const InfoIcon = infoIconMap[item.icon] || Phone;

                                    return (
                                        <div
                                            key={item.id}
                                            className={`flex items-start gap-4 ${idx < info.items.length - 1 ? 'border-b border-[#e2e8e0] pb-6' : ''
                                                }`}
                                        >
