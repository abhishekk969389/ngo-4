...
export default function Articles() {
  ...
  const getIcon = (icon: string) => {
    const IconComponent =
      iconMap[icon as keyof typeof iconMap] ?? iconMap.default;
  ...
  const headingWords = articlesData.heading
    ? articlesData.heading.split(" ")
    : [];
  const primaryTitle = headingWords.slice(0, -1).join(" ");
  const highlightedTitle =
    headingWords.length > 1 ? headingWords[headingWords.length - 1] : "";
    ...
    <section className="relative overflow-hidden bg-white ">
      <div className="pointer-events-none absolute left-0 top-1/2 h-40 w-16 -translate-y-1/2 opacity-25">
        <svg
          viewBox="0 0 100 200"
          fill="currentColor"
          className="h-full w-full text-[#3b6043]"
        >
